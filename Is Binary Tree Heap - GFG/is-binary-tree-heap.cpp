...
bool maxOrderProperty(struct Node* root)
{
    ...
    if(root -> right == NULL)
    {
        ...
        bool left = maxOrderProperty(root -> left) ;
        bool right = maxOrderProperty(root -> right) ;
...
class Solution {
  public:
    bool isHeap(struct Node* tree) {
        ...
        if(isCBT(tree , 0 , count) && maxOrderProperty(tree))
        {
            return true ;
        }
        else{
